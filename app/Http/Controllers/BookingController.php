...
class BookingController extends Controller
{
     ...
     * Webhook thanh toán → xác nhận paid
     */
    public function confirm(Request $request, int $id): JsonResponse
    {
        $validated = $request->validate([
            'payment_method' => 'required|string|in:momo,vnpay,zalopay,bank_transfer',
        ]);

        try {
            $booking = $this->bookingService->confirmPayment($id, $validated['payment_method']);
    ...
    public function checkSlotStatus(Request $request): JsonResponse
    {
        ...
        foreach ($slots as $slot) {
            ...
            $isBooked = $bookedSlots->contains(function ($b) use ($slot) {
                return $b->field_id    == $slot['field_id']
